...
RSpec.describe User, type: :model do
  ...
  describe 'is associated to posts, comments, likes' do
  ...
  describe 'can request friendship and accept/reject them' do
    ...
    it 'requests friendship' do
      ...
      expect(Friendship.where(invitation_sender_id: 1, invitation_receiver_id: 2)).not_to eq nil
    end

    it 'accepts friendship' do
      user1.friendship_request(user2)
      user2.accept_request(user1)
      expect(Friendship.where(invitation_sender_id: 1, invitation_receiver_id: 2).first.status).to eq(true)
    end

    it 'rejects friendship' do
      user1.friendship_request(user2)
      user2.reject_request(user1)
      expect(Friendship.where(invitation_sender_id: 1, invitation_receiver_id: 2).first).to eq nil
    end
  end
end
